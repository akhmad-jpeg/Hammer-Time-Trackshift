...
def import_dataset(
    ...
    driver_id      : Permanent F1 driver id (= drivers.driver_id in the DB).
    session_type   : "R", "Q", "FP1", "FP2", or "FP3".
    allow_duplicate: If False, skip sessions already in the DB.
    driver_code    : 3-letter code (e.g. "VER").  Passed to import_race so the
                     driver is matched inside each session by code -- race
                     numbers change across seasons (champion takes #1, etc.)
                     and can collide between years.
    """
    logging.info("=" * 60)
