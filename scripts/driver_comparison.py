...
from feature_pipeline import covered_tracks, covered_tyres, construct_prediction_input
...
def _predict(model, feature_names, track, tyre, age, lap_number=25, year=None):
    """Predict one lap time with a driver's model on an aligned feature row.

    Goes through construct_prediction_input so the race-phase bucket, era
    bucket and the tyre_age x compound interactions are populated exactly as
    in training.  The phase anchor defaults to a mid-race lap (25) and the
    era to the caller's year (or the default era) — comparisons are
    within-model deltas at identical inputs, so the anchors cancel out.
    """
...
def compare_drivers(code_a, code_b, ages=DEFAULT_AGES, models_dir=None, year=None):
    ...
    for track in shared_tracks:
        ...
        for tyre in shared_tyres:
            rows = []
            for age in ages:
                ta = _predict(model_a, feats_a, track, tyre, age, year=year)
                tb = _predict(model_b, feats_b, track, tyre, age, year=year)
                rows.append({
                    "age": int(age),
                    "driver_a": round(ta, 3),
                    "driver_b": round(tb, 3),
