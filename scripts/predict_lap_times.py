...
# Optional season year (sets the model's era bucket; blank = the era with
# the most training data).  Cars get faster over seasons, so an accurate
# year gives the most representative lap time.
print("\n" + "=" * 60)
print("SEASON YEAR (optional)")
print("=" * 60)
_year_raw = input("Season year (e.g. 2024; blank for default era): ").strip()
year = None
if _year_raw.isdigit():
    year = int(_year_raw)
    print(f"  Using {year} era.")
else:
    print("  Using the most-representative era from training data.")

# Create input DataFrame with ALL features from training (race-phase bucket,
# era bucket and tyre_age x compound interactions included by the shared
# constructor).
