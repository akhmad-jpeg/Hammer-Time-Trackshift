...
RACE_CALENDAR = {
    ...
    2025: ["Australia", "China", "Japan", "Bahrain", "Saudi Arabia", "Miami",
           ...
           "United States", "Mexico", "Brazil", "Las Vegas", "Qatar", "Abu Dhabi"],
    # 23-race 2026 calendar, verified against FastF1's 2026 schedule: Barcelona
    # (r7) and Madrid (r14) are separate rounds, and there is no Saudi/Imola GP.
    2026: ["Australia", "China", "Japan", "Miami", "Canada", "Monaco", "Barcelona",
           "Austria", "Silverstone", "Belgium", "Hungary", "Netherlands", "Italy",
           "Spain", "Azerbaijan", "Bahrain", "Singapore", "United States", "Mexico",
...
def print_driver_menu(drivers: list[dict]) -> None:
    ...
    print("  " + "-" * 40)
    print("  (Numbers change across seasons - the champion takes #1 and drivers")
    print("   renumber.  Imports match by 3-letter code, so pick the driver row")
    print("   and any year works.)")
    for d in drivers:
...
def print_track_menu(year: int) -> list[str]:
    ...
    if not calendar:
        print(
            f"  (No numbered calendar for {year}. Entering a NUMBER here will not\n"
            f"   resolve - type a race name instead, e.g. 'Monza' or 'Australia'.)"
        )
        return []
...
def resolve_race_input(raw_input: str, calendar: list[str]) -> str:
    ...
    if token.isdigit() and not calendar:
        # A bare number with no menu would be passed to FastF1 as a race
        # "name" and fuzzy-matched to an arbitrary event (historically the
        # first race of the season, e.g. Australian GP). Refuse instead.
        raise ValueError(
            f"'{token}' looks like a menu number, but this season has no numbered "
            f"calendar. Type a race name instead, e.g. 'Monza' or 'Australia'."
        )
...
def import_race(year: int, race_name: str, driver_id: int,
                session_type: str = "R", allow_duplicate: bool = False,
                driver_code: str | None = None) -> int | None:
    """
    ...
    driver_id     : Permanent driver id in the DB (= drivers.driver_id).  This
                    is NOT necessarily the number the driver wears in ``year``:
                    the champion takes #1 and drivers change numbers between
                    seasons, so identity is resolved by 3-letter code whenever
                    ``driver_code`` is supplied.
    session_type  : "R" (Race), "Q" (Qualifying), "FP1", "FP2", "FP3".
    allow_duplicate : If False (default) skip if session already in DB.
    driver_code   : Optional 3-letter code (e.g. "VER").  When supplied it is
                    the PRIMARY key used to find the driver inside the FastF1
                    session (numbers are re-issued and can collide across
                    years); driver_id is matched by number only as a fallback
                    for callers that cannot provide a code.  All rows are
                    still written under the permanent driver_id, so a driver
                    is never duplicated when their race number changes.
    ...
    # ------------------------------------------------------------------
    # A driver's race number is NOT a stable identity: the defending
    # champion takes #1 and drivers change numbers between seasons (VER raced
    # as #33 in the 2021 imports but is #3 on this 2026 grid, and a number a
    # DB driver used in 2021 may belong to a DIFFERENT driver today).  The
    # 3-letter code is the stable key, so when a code is supplied we match on
    # it first and fall back to the legacy numeric lookup only for callers
    # that cannot provide one.
    # FastF1 swallows failures inside Session.load for individual datasets and
    # only logs warnings, so a session with NO data at all (e.g. a future
    # round that has not been raced yet) leaves .laps unloaded - accessing it
    # then raises DataNotLoadedError.  Translate that into a clear per-race
    # error instead of letting it crash upward through the batch runner.
    try:
        session_laps = session.laps
    except Exception as exc:
        logging.error(
            f"No data available for {year} {race_name} ({session_type}): {exc}. "
            f"The session may not have taken place yet - rounds later in the "
            f"season have no data until the race has been run. Aborting."
        )
        return None
    ...
    if target_code:
        driver_laps = session_laps[
            session_laps["Driver"].astype(str).str.strip().str.upper() == target_code
        ]
    else:
        driver_laps = session_laps[
            session_laps["DriverNumber"].astype(str) == str(driver_id)
        ]

    if driver_laps.empty:
        present = sorted(session_laps["DriverNumber"].dropna().unique().tolist())
        logging.error(
            f"Driver #{driver_id}"
            + (f" ({target_code})" if target_code else "")
            + f" not found in the FastF1 session (drivers present: {present}). "
            + "The driver may not have raced, or may run under a different number "
            + "this season - pass the 3-letter code to match by identity instead. "
            + "Aborting."
        )
        return None
    ...
    if target_code and fastf1_number != driver_id:
        logging.info(
            f"Driver {fastf1_code} wears #{fastf1_number} in this session but is "
            f"stored as #{driver_id} in the DB - matched by code; keeping the "
            f"permanent DB id #{driver_id} for all rows."
        )

    logging.info(f"Resolved driver: #{driver_id} {fastf1_name} ({fastf1_code})")
    ...
    try:
        ...
        # Ensure the permanent driver row exists (won't overwrite existing
        # name).  Keyed on driver_id - the DB identity - NOT fastf1_number, so
        # a driver matched by code under a different race number is never
        # duplicated in the drivers table.
        upsert_driver_from_fastf1(cursor, driver_id, fastf1_code, fastf1_name)
        ...
        for _, lap in driver_laps.iterrows():
            try:
                ...
                # Sector split times come straight off the FastF1 lap row as
                # Timedelta objects (ms resolution).  They can be NaN on
                # out-laps / deleted laps -- keep NULL then.
...
def _interactive_prompt() -> dict:
    ...
    return dict(
        ...
        driver_code=db_driver["driver_code"],
...
if __name__ == "__main__":
    ...
    if use_interactive:
        ...
        if args.race is None:
            print("ERROR: --race is required in CLI mode (race name or menu number).")
            sys.exit(1)
        # A bare menu number is resolved against the printed calendar in
        # interactive mode; do the same here so `--race 10` can never be
        # fuzzy-matched to the wrong event inside FastF1.
        if args.race.strip().isdigit():
            try:
                race_name = resolve_race_input(args.race, RACE_CALENDAR.get(args.year, []))
            except ValueError as exc:
                print(f"ERROR: {exc}")
                sys.exit(1)
        else:
